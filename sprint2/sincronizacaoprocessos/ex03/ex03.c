...
int main(int argc, char *agrv[]) {
    ...
    if(p>0){
        ...
        sprintf(dataPointer->arrayString[p], "Sou o filho n. %d e com o processo n.%d", p, getpid());
    ...
    //processo pai
//esta escrita seria apenas para simular a string do enunciado, para realmente escrever "sou o pai"
    sem_wait(semaforo);
    sprintf(dataPointer->arrayString[p], "Sou o pai n. %d e com o processo n.%d", p, getpid());
    //int n = rand() % 5;
    //sleep(n);
    sleep(1);
    //incrementar semaforo
    ...
    for (i = 0; i< 49; i++) { // espera os filhos para iniciar
        wait(NULL);
    }
